/// Evaluate response assertions against FHIR server responses.
///
/// Ported from fhir-autotest's runner/response_assertions.rs.
/// Validates Bundle structure, entry counts, field values, sort order,
/// include types, absent fields, OperationOutcome severity, and
/// mustSupport field presence.
use serde_json::Value;

use super::test_model::ResponseAssertion;
...
/// Resolve a dotted JSON path like "name.family" or "name[0].family" to a value.
...
fn resolve_json_path(value: &Value, path: &str) -> Option<Value> {
    ...
    for part in parts {
        // Handle array indexing: "name[0]" → get "name" then index 0
...
mod tests {
    ...
    fn assert_min_entries_fail() {
        ...
        let errors = assert_response(&assertion, 200, &Some(body));
        assert!(errors.iter().any(|e| e.contains("1") && e.contains("2")));
    }

    #[test]
    fn assert_required_fields_pass() {
    ...
    #[test]
    fn assert_required_fields_fail() {
        let mut required = HashMap::new();
        required.insert("Patient".to_string(), vec!["birthDate".to_string()]);
        let assertion = ResponseAssertion {
            required_fields: required,
            ..ResponseAssertion::none()
        };
    ...
    fn assert_field_values_match() {
        ...
        fields.insert("name[0].family".to_string(), json!("Smith"));
        field_values.insert("Patient".to_string(), fields);
        let assertion = ResponseAssertion {
            field_values,
            ..ResponseAssertion::none()
        };
    ...
    fn assert_sort_ascending() {
        let assertion = ResponseAssertion {
            sort_by: Some(super::super::test_model::SortAssertion {
        ...
        let body = json!({
            ...
            "entry": [
                ...
                {"resource": {"resourceType": "Patient", "id": "2", "birthDate": "2000-01-01"}},
                {"resource": {"resourceType": "Patient", "id": "3", "birthDate": "2010-01-01"}}
            ]
        });
        let errors = assert_response(&assertion, 200, &Some(body));
        assert!(errors.is_empty(), "Expected no errors, got: {:?}", errors);
    ...
    fn assert_absent_fields() {
        let assertion = ResponseAssertion {
            absent_fields: vec!["birthDate".to_string()],
            ..ResponseAssertion::none()
        };
        let body = json!({
            "resourceType": "Bundle",
