...
- datatype-null -> element set to JSON null.
...
Note: `invariant-violates` is not derived. Constructing a concrete violation
would require evaluating the FHIRPath invariant expression, which is not yet
implemented; nulling the subject element does not break most invariants (and is
often just an absence accepted for optional elements), so it was removed to
avoid generating a reject test that a conformant server accepts and therefore
falsely fails. Only the positive `invariant-satisfies` obligation is derived.
Length-bounded string obligations still await length-constraint modelling; the
boundary helpers are there to consume it.
