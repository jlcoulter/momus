...
// sources, i.e. none). rootPath is the resource file path, used to locate the
// package next to it when --package is relative.
func buildRegistryForValidate(cfg *config, _ string) (*registry.Registry, error) {
	if cfg.packagePath == "" {
		return registry.New(), nil
	}
	graph, reg, err := resolvePackageGraph(cfg, cfg.packagePath)
	if err != nil {
