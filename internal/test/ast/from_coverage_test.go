...
func TestGenerateFromCoveragePlanBuildsPerRequirementSequence(t *testing.T) {
	...
	resourceSeq := root.Steps[0].(*Sequence)
	setupReq := resourceSeq.Steps[0].(*Request)
	...
	case0 := resourceSeq.Steps[3].(*Sequence)
	caseReq := case0.Steps[0].(*Request)
...
func TestEncodePlanIncludesTypeTags(t *testing.T) {
	plan := &Plan{Version: "v1", Root: &Sequence{Steps: []Node{&Request{Method: "GET", URL: "/Patient"}, &Assert{Description: "ok", RequirementID: "r-1", Expression: "status == 200"}}}}
...
func TestGenerateFromCoveragePlanPopulatesRequiredIdentifierSlicesFromProfile(t *testing.T) {
	...
	root := plan.Root.(*Sequence)
	resourceSeq := root.Steps[0].(*Sequence)
	caseSeq := resourceSeq.Steps[3].(*Sequence)
	req := caseSeq.Steps[0].(*Request)
